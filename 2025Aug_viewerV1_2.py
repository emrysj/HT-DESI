...
class WellPlate(QWidget):
    ...
    def setup_custom_plate(self, active_wells):
        ...
        print(f"Setting up custom plate with {len(active_wells)} wells")
        print(f"Well positions available: {len(self.custom_well_positions) if self.custom_well_positions else 0}")
        
        self.clear_plates()
        self.active_wells = set(active_wells)
        
        if not self.custom_well_positions:
            print("No custom well positions available!")
            return
        ...
        print(f"Grid size: {max_row + 1} rows x {max_col + 1} columns")
        ...
        for pos in self.custom_well_positions:
            ...
            print(f"Creating button for well {well_id} at position ({row}, {col})")
        ...
        # Fill empty positions with spacers for visual clarity
        ...
        print(f"Custom plate setup complete. Created {len(self.buttons)} buttons.")
