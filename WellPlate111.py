# -*- coding: utf-8 -*-
"""
Created on Tue Nov 21 17:02:22 2023
...
class PlateModel(object):
    ...
    def list(self):
        ...
        for key, label in self.labels.items():
            print ('%s (Column Major):' % label, self.get_col_major_indexes(key))
            print ('%s (Row Major):' % label, self.get_row_major_indexes(key))
            print ('%s (Names):' % label, self.get_names(key))
       
            data = self.get_col_major_indexes(key)
            print(data)
            f = open("C:/Users/ukejon/OneDrive - Waters Corporation/Desktop/test.csv", 'w')
            writer = csv.writer(f)
...
class PlateDialog(wx.Dialog):
    ...
    def create_buttons(self, parent):
        ...
        list = wx.Button(parent, -1, 'List')
        list.Bind(wx.EVT_BUTTON, self.on_select_list)
